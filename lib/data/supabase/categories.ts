...
export async function getCategoryAttributes(client: MarketoSupabaseClient, categoryId: string) {
  const { data, error } = await client
    ...
    .select("id, category_id, key, label_ru, label_kk, data_type, unit_ru, unit_kk, is_required, is_filterable, is_searchable, inherits_to_children, validation, sort_order")
...
export async function listAttributeOptions(client: MarketoSupabaseClient, attributeIds: string[]) {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data, error } = await client
      ...
      .select("id, attribute_id, value, label_ru, label_kk, sort_order")
