...
export class OpenAIModerationProvider implements MultimodalModerationProvider {
  ...
  async analyzeListing(input:AIInput,parent:AbortSignal){
    ...
    if(input.images.length>7||input.images.some(i=>i.bytes.length>512*1024||i.mimeType!=='image/jpeg')||new Set(input.images.map(i=>i.image_index)).size!==input.images.length||input.title.length>120||input.description.length>20_000||input.attributes.length>12_000||input.category.length>3000)throw new AIProviderError('content_unavailable');
    ...
    try{
      const response=await this.transport('https://api.openai.com/v1/responses',{method:'POST',redirect:'manual',signal:controller.signal,headers:{authorization:`Bearer ${this.key}`,'content-type':'application/json'},body:JSON.stringify({
        ...
        text:{format:{type:'json_schema',name:'moderation_ai_observation_v1',strict:true,schema}},
      })});
