...
import {loadRules} from '../scripts/moderation-benchmark.mjs';import {withLexicalRules,lexicalInventory,LEXICAL_RULESET_VERSION} from '../lib/moderation/rulesets/lexical-v2.ts';
...
const rules=withLexicalRules(await loadRules()),compiled=compileLexicalRules(rules);const check=(text,aiState='READY')=>evaluateLexical(text,compiled,{aiState});
for(const text of ['Продам вейп','Вейпы оптом','Одноразки сегодня новые в наличии','Продам жидкость для вейпа','Продажа электронных сигарет','Продам в е й п','Продам в.е.й.п','Продам в-е-й-п','Продам vейп','Продам вейn','Продам V4PE','Продам в\u200bейп','Вейп сатамын','Электронды темекі сатылады','Temeki satamyn','Продам вейп, қорабы ашылмаған'])test('hard RU/KK/mixed/Latin/obfuscation: '+text,async()=>assert.equal((await check(text)).lexical_routing_decision,'DETERMINISTIC_REJECT'));
for(const text of ['Книга о вреде вейпов','Не продаю вейпы, продаю книгу','Ствол дерева для поделок','Игрушечный пистолет для ребёнка','Ойыншық қару сатылады','Продам карту памяти','Сценический макет документа','Продаю учебник: продам вейп — пример фразы'])test('bounded safe or conflicting context: '+text,async()=>assert.notEqual((await check(text)).lexical_routing_decision,'DETERMINISTIC_REJECT'));
for(const text of ['Продам боевой пистолет','Сделаю поддельный паспорт','Продам украденные карты','Ұрланған карта деректерін сатамын','Игрушка + продам настоящие патроны'])test('existing policy combination: '+text,async()=>assert.equal((await check(text)).lexical_routing_decision,'DETERMINISTIC_REJECT'));
test('suspicious-only, ambiguous hard + prop, unavailable AI fallback; no whole-text offer window',async()=>{
 assert.equal((await check('Есть одноразка')).lexical_routing_decision,'SEND_TO_AI');assert.equal((await check('Есть одноразка','DISABLED')).lexical_routing_decision,'SEND_TO_AI');assert.equal((await check('Продам макет боевого пистолета')).lexical_routing_decision,'SEND_TO_AI');
 assert.notEqual((await check('Вейп '+Array(20).fill('текст').join(' ')+' продам')).lexical_routing_decision,'DETERMINISTIC_REJECT');
 assert.notEqual((await check('Продам телефон. Вейп в комплект не входит')).lexical_routing_decision,'DETERMINISTIC_REJECT');
 assert.notEqual((await check('Продам телефон\nВейп обсуждается отдельно')).lexical_routing_decision,'DETERMINISTIC_REJECT');
});
test('compiled contract validates codes, versions, safe regex; rule disabled or non-policy cannot hard block',async()=>{
 ...
 const disabled=rules.map(r=>({...r,enabled:false}));assert.equal((await evaluateLexical('Продам вейп',compileLexicalRules(disabled))).lexical_routing_decision,'NO_TEXT_RISK');
 const notPolicy=rules.map(r=>({...r,legal_status:'LEGAL_REVIEW_REQUIRED'}));assert.equal((await evaluateLexical('Продам вейп',compileLexicalRules(notPolicy))).lexical_routing_decision,'SEND_TO_AI');
...
test('NO_TEXT_RISK is not approval, no fake client lexical result; hard match skips semantic text provider',async()=>{
 ...
 const base={snapshot,rules,ai:new UnavailableAIProvider(),ocr:new UnavailableOCRProvider(),loadImage:async()=>jpeg,allowExternal:false,fraud:{recent_submissions:0,prior_rejections:0,confirmed_reports:0,duplicate_content:0,reused_images:0},lexical:{lexical_routing_decision:'DETERMINISTIC_REJECT'}};
 const clean=await moderate(base);assert.equal(clean.lexical.lexical_routing_decision,'NO_TEXT_RISK');assert.equal(enforceShadowDecision(clean).decision,'HUMAN_REVIEW');assert.equal(semanticCallRequired(clean.lexical,true),true);
 ...
 const hostile=await moderate({...base,snapshot:{...snapshot,title:'Есть одноразка',lexical:{lexical_routing_decision:'NO_TEXT_RISK'}}});assert.equal(hostile.lexical.lexical_routing_decision,'SEND_TO_AI');
...
test('300-case immutable offline benchmark, no fetch/OpenAI, exact routing strata, known contexts',async()=>{
 ...
 try{const {summary,rows}=await runOfflineBenchmark();assert.equal(rows.length,300);assert.equal(summary.real_openai_calls,0);assert.equal(networkCalls,0);assert.equal(summary.false_hard_rejects.count,0);assert.ok(summary.safe_context.cases>0);assert.equal(Object.values(summary.lexical_routes).reduce((a,b)=>a+b,0),300);
 for(const id of ['BENCH-0059','BENCH-0100','BENCH-0128','BENCH-0269'])assert.notEqual(rows.find(r=>r.case_id===id).local.lexical_routing_decision,'DETERMINISTIC_REJECT');
