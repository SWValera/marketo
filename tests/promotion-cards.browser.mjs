...
try{
 ...
 for(const [width,height] of [[390,844],[430,932],[844,390],[768,1024],[1180,820],[1366,768]]){
  ...
  const geometry=await evaluate(`(()=>{const grid=document.querySelector('.listing-grid'),g=getComputedStyle(grid),columns=g.gridTemplateColumns.split(' ').map(parseFloat),gap=parseFloat(g.columnGap),cards=[...grid.children].map(c=>{const b=c.getBoundingClientRect(),media=c.querySelector('.listing-image-wrap').getBoundingClientRect(),s=getComputedStyle(c);return {width:b.width,height:b.height,span:s.gridColumnStart,body:getComputedStyle(c.querySelector('.listing-body')).backgroundColor,badge:!!c.querySelector('.listing-vip-badge'),imageFit:getComputedStyle(c.querySelector('img')).objectFit,ratio:media.width/media.height};});return {columns,gap,cards,overflow:document.documentElement.scrollWidth>innerWidth};})()`);
  const [normal,vip,x2,both]=geometry.cards;assert.equal(geometry.overflow,false);assert.ok(Math.abs(normal.width-vip.width)<1);assert.ok(Math.abs(normal.height-vip.height)<1);
  assert.equal(vip.body,'rgb(221, 245, 233)');assert.equal(both.body,vip.body);assert.equal(normal.body,'rgb(255, 255, 255)');assert.equal(x2.body,normal.body);assert.equal(vip.badge,true);assert.equal(both.badge,true);assert.equal(normal.badge,false);
 ...
 await send('Page.navigate',{url:origin+'/?expire'});await until("document.querySelectorAll('.listing-card-vip').length===2");await until("document.querySelectorAll('.listing-card-vip,.listing-card-x2').length===0");assert.equal(await evaluate("document.querySelectorAll('.listing-card').length"),4);
