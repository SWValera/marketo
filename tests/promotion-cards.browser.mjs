...
const entry=`import React from 'react';import{createRoot}from'react-dom/client';import{ListingCard}from'./components/listing-card';const base=${JSON.stringify(base)};const end=new Date(Date.now()+(location.search.includes('expire')?1800:86400000)).toISOString();base.expiresAt=new Date(Date.now()+86400000).toISOString();const items=[{...base,id:'normal',title:'Планшет m5'},{...base,id:'vip',vipUntil:end},{...base,id:'x2',x2Until:end},{...base,id:'vip-x2',vipUntil:end,x2Until:end}];createRoot(document.getElementById('app')).render(<main className='page-shell'><div className='listing-grid'>{items.map(x=><ListingCard key={x.id} listing={x}/>)}</div></main>);`;
...
try{
 ...
 // Changing only device wall time must not remove cards or active VIP/X2.
 await send('Page.navigate',{url:origin});await until("document.querySelectorAll('.listing-card-vip').length===2");
 await evaluate("window.originalNow=Date.now;Date.now=()=>window.originalNow()+40*86400000;window.dispatchEvent(new Event('focus'))");await delay(100);
 assert.equal(await evaluate("document.querySelectorAll('.listing-card').length"),4);assert.equal(await evaluate("document.querySelectorAll('.listing-card-vip').length"),2);
 await evaluate("Date.now=()=>window.originalNow()-40*86400000;document.dispatchEvent(new Event('visibilitychange'))");await delay(100);
 assert.equal(await evaluate("document.querySelectorAll('.listing-card-vip').length"),2);
