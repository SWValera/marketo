...
test('built auth recovery and signup redirects work in workerd with isolated session cookies',async()=>{
 ...
 let fail=false,verifications=0,users=0,exchanges=0;
 mock.get(fixtureOrigin).intercept({path:'/auth/v1/token?grant_type=pkce',method:'POST'}).reply(()=>{exchanges++;return {statusCode:200,data:JSON.stringify(session),responseOptions:{headers:{'content-type':'application/json'}}};}).persist();
 mock.get(fixtureOrigin).intercept({path:'/auth/v1/verify',method:'POST'}).reply(()=>{verifications++;return {statusCode:fail?403:200,data:JSON.stringify(fail?{code:'otp_expired',msg:'Email link is invalid or has expired'}:session),responseOptions:{headers:{'content-type':'application/json'}}};}).persist();
 ...
 try{
  ...
  fail=true;
  for(let i=0;i<2;i++){
   const used=await mf.dispatchFetch('https://jevu.kz/auth/callback?type=recovery&token_hash=fixture',{redirect:'manual'});assert.equal(used.status,307);assert.match(used.headers.get('location'),/auth_error=expired/);assert.equal(used.headers.getSetCookie().length,0);
  }
  ...
  fail=false;
  const signup=await mf.dispatchFetch('https://jevu.kz/auth/callback?type=signup&token_hash=fixture',{redirect:'manual'});assert.equal(signup.status,307);assert.match(signup.headers.get('location'),/^https:\/\/jevu.kz\/auth\/result\?event=signup-confirmed/);
