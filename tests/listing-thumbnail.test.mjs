...
function setup({isPublic=true,authenticated=true,privateRow=true,processorError=false}={}){
 ...
 state.processor={input(stream){let resize;return {transform(options){resize=options;state.transforms.push(options);return this},async output(options){state.outputs.push(options);if(processorError)throw Error("processor down");const bytes=Buffer.from(await new Response(stream).arrayBuffer());const out=await sharp(bytes).resize(resize.width,resize.height,{fit:resize.fit==="scale-down"?"inside":resize.fit,withoutEnlargement:resize.fit==="scale-down"}).webp({quality:options.quality}).toBuffer();return{response:()=>new Response(out,{headers:{"content-type":"image/webp"}})}}}}};
...
test("authorized thumbnail decodes to 768x576 WebP; no writes or original response",async()=>{
 ...
 assert.ok(bytes.length<original.length);assert.match(thumb.headers.get("cache-control"),/private, no-cache/);
...
test("public conditional reuse verifies eligibility and avoids another transform",async()=>{
 const state=setup();const first=await get();await first.arrayBuffer();
 const r=await GET(new Request(url+"?variant=card",{headers:{"if-none-match":first.headers.get("etag")}}),{params:Promise.resolve({key:key.split("/")})});
 assert.equal(r.status,304);assert.equal((await r.arrayBuffer()).byteLength,0);assert.equal(state.transforms.length,1);
 setup({isPublic:false,authenticated:false,privateRow:false});
 assert.equal((await GET(new Request(url+"?variant=card",{headers:{"if-none-match":first.headers.get("etag")}}),{params:Promise.resolve({key:key.split("/")})})).status,404);
});

test("edge variant reuse stays behind live authorization and object version",async()=>{
 const entries=new Map();globalThis.caches={default:{match:async r=>entries.get(r.url)?.clone(),put:async(r,v)=>{entries.set(r.url,new Response(await v.arrayBuffer(),{headers:v.headers}))}}};
 try {
  const state=setup();const first=await get();await first.arrayBuffer();assert.equal(first.headers.get("x-media-cache"),"miss");
  const second=await get();await second.arrayBuffer();assert.equal(second.headers.get("x-media-cache"),"hit");assert.equal(state.transforms.length,1);
  setup({isPublic:false,authenticated:false,privateRow:false});assert.equal((await get()).status,404);
  const next=setup();next.bucket.get=async()=>({body:new Response(original).body,httpEtag:'"changed"'});
  const changed=await get();await changed.arrayBuffer();assert.equal(changed.headers.get("x-media-cache"),"miss");assert.equal(next.transforms.length,1);
 }finally{delete globalThis.caches}
});

test("detail preview preserves the entire photograph and remains bounded",async()=>{
 const state=setup();const response=await get("?variant=detail");
 ...
 assert.deepEqual(state.transforms,[{width:1024,height:1024,fit:"scale-down"}]);
