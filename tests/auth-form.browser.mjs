...
const server=createServer((req,res)=>{
 ...
 else if(path==='/api/auth/registration/start'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({callback:'https://jevu.kz/auth/callback?flow=signup&bridge=fixture'}));}
 ...
 else if(path==='/auth/callback'){res.writeHead(307,{location:'/auth/update-password','cache-control':'no-store'});res.end();}
...
try{
 ...
 for(const mobile of [false,true]){
  ...
  for(const scenario of ['new','existing']){
   ...
   assert.deepEqual(await evaluate('window.fixture.calls'),['signUp']);assert.equal(await evaluate(`document.querySelectorAll('input[type="password"]').length`),0);
  ...
  for(const error of ['expired','invalid']){
   await show('recover','new',{callbackError:error},'signup');assert.match(await text(),error==='expired'?/Срок действия ссылки истёк/:/Ссылка недействительна/);assert.equal(await evaluate(`!!document.querySelector('.auth-pending-state')`),false);assert.equal(await evaluate(`document.querySelectorAll('input[type="password"]').length`),0);report.push({mobile,scenario:'callback-'+error,status:'PASS'});
  }
  ...
  await show('recover');await fill('input[type="email"]','fixture@example.test');await submit();assert.match(await text(),/Если аккаунт с этим адресом существует/);assert.doesNotMatch(await text(),/Подтвердить регистрацию/);
 ...
 // Browser actually controlled by this exact SW: auth bypass, generic opaque redirect, and offline fallback.
 ...
 for(const [path,target,native] of [['/auth/callback?flow=recovery&code=fixture','/auth/update-password',true],['/login?mode=recover','/login',true],['/guarded-fixture','/login',true]]){
 ...
 await send('Page.navigate',{url:url+'/ordinary-offline-fixture'});await until(`document.body.innerText.includes('OFFLINE FIXTURE')`);report.push({scenario:'offline preserved',status:'PASS'});
