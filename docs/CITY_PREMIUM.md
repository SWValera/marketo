...
The publication wizard still submits for moderation. Its success screen offers the optional service; activation is available after approval from the owner's profile card. The profile panel loads only when expanded, without adding work to server account-page rendering. RU/KK shows configured price, duration, availability, full/error/pending states and current/expired status. Promotion errors do not undo publication.
