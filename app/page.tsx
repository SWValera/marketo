...
import { getCategoryReferences, getGeographyReferences } from "@/lib/reference-data/server";

export const metadata: Metadata = { alternates: { canonical: "/" } };

export default async function Home() {
  const [{ locale, t }, catalog, geography] = await Promise.all([
    getServerI18n(),
  ...
  return <>
    ...
    <main>
      <section className="hero-shell page-shell">
        <div className="category-sidebar"><div className="sidebar-heading">{t("admin.categories")}</div><nav aria-label={t("categories.aria")}>{rootCategories.map((category) => <Link key={category.id} href={`/category/${category.slug}`}><CategoryIcon name={category.icon ?? undefined} size={17} /><span>{localize(category.name, locale)}</span></Link>)}</nav></div>
        <div className="hero-main">
          <div className="hero-card"><div className="hero-copy"><span className="eyebrow"><Sparkles size={15} /> {t("home.eyebrow")}</span><h1>{t("home.title")}</h1><p>{t("home.description")}</p><div className="hero-actions"><Link href="/publish" className="primary-action">{t("header.publish")} <ArrowRight size={18} /></Link><Link href="/search" className="secondary-action">{t("home.viewCatalog")}</Link></div><div className="hero-stats"><div><strong>{geography.data.settlements.length}</strong><span>{t("home.cities")}</span></div><div><strong>{geography.data.regions.length}</strong><span>{t("home.regions")}</span></div><div><strong>2</strong><span>{t("home.languages")}</span></div></div></div><div className="hero-visual" aria-hidden="true"><div className="visual-orbit orbit-one"><Heart size={23} /></div><div className="visual-orbit orbit-two"><MapPin size={25} /></div><div className="visual-phone"><div className="visual-phone-top" /><div className="mini-search"><Search size={12} /> {t("common.search")}...</div><div className="mini-card large" /><div className="mini-grid"><div /><div /></div></div></div></div>
